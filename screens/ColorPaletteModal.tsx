...
import { Color, MainStackParamList, RootStackParamList } from '../App';
...
export const ColorPaletteModal: React.FC<Props> = ({ navigation }) => {
  ...
  const handleSubmit = useCallback(() => {
    if (!name) {
      ...
      return Alert.alert('Please enter at least 3 colors');
    }
  ...
  const handleValueChange = useCallback((value: boolean, color: Color) => {
    if (value === true) {
      return setSelectedColors((colors) => [...colors, color]);
    }
